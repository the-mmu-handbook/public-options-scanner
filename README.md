...
- Greeks stream progressively — ATM-closest contracts load first
...
Each leg has a **strike dropdown** and **expiry dropdown**. Changing the expiry re-fetches that chain and auto-matches the closest strike.
...
**Order flow**: Build → Preflight → Confirm → Place.
...
### 📈 Analytics
- **IV Smile** — implied volatility vs strike for calls and puts; shows skew
- **Max Pain** — strike where option buyers face maximum dollar loss at expiry
- **Put/Call Ratio** — call OI vs put OI per strike with aggregate sentiment label
- **Implied Move** — ATM straddle price ÷ spot = expected ±% move by expiry
- **Volatility Surface** — pseudo-3D IV mesh across all strikes and expirations; heat-coloured, loads progressively
...
- Preflight must pass before Place Order enables
- No auto-trading — every order requires explicit confirmation
